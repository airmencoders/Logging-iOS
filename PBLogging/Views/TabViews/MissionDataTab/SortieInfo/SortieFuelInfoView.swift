...
struct SortieFuelEditFields: View {
    ...
    init(sortie: Sortie){
        ...
        _rampFuel   = State(wrappedValue: sortie.fuel.ramp == 0 ?
                                          "" : "\(sortie.fuel.ramp)")
        _landFuel   = State(wrappedValue: sortie.fuel.land == 0 ?
                                          "" : "\(sortie.fuel.land)")
        _airRefuel  = State(wrappedValue: sortie.fuel.airRefuel == 0 ?
                                          "" : "\(sortie.fuel.airRefuel)")
        _auxPower   = State(wrappedValue: sortie.auxiliaryPowerUnitHours == 0 ?
                                          "" : "\(sortie.auxiliaryPowerUnitHours)")
        _cog        = State(wrappedValue: sortie.takeoffCenterOfGravity == 0 ?
